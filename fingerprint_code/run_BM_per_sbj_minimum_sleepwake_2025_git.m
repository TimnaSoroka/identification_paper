function C=run_BM_per_sbj_minimum_sleepwake_2025_git(AllSubjData,directory,WSA,normalize,overlap,cleaning,block_length,train_by_hours,prc_train,timings,start,rand)
...
for iii=1:length(timings)
   ...
   %     load([ directory '/fingerprint_code/values1shorts.mat']);
    ...
    for i=1:size(AllSubjData,1)
        ...
        %SubjectName=subjData.Name;
            SubjectName=subjData.folder(end-7:end);
                        ...
                        file=dir([directory '/Data/sum_' num2str(block_length) 'min_blocks_per_subj/' SubjectName '/' SubjectName '*BM_sleep.mat']);
        load([file.folder '/' file.name]);
size_sleep_mat=size(mat,2);
clearvars mat labels

                        file=dir([directory '/Data/sum_' num2str(block_length) 'min_blocks_per_subj/' SubjectName '/' SubjectName '*BM_wake.mat']);
        load([file.folder '/' file.name]);
clearvars labels
        ...
        for ii=1:length(o)
            ...
            Data_per_subject=Data_per_subject(1:size_sleep_mat,:);
...
%                 save([directory '/fingerprint_code/result_BM_same_day/acc_id_BM_' WSA '_' num2str(block_length) 'min_blocks_'  num2str(prc_train) '_' num2str(timings(iii)) 'min_rand_' num2str(size(AllSubjData,1)) '_' datestr(datetime) '.mat'],'C','validationAccuracyReal',"labels_hat_all", 'testing_labels_all');
               ...
               save([directory '/fingerprint_code/result_BM_same_day/acc_id_BM_' num2str(block_length) 'min_blocks_' num2str(prc_train) '_' num2str(timings(iii)) 'min_' num2str(size(AllSubjData,1)) 'wake_capped.mat'],'C','validationAccuracyReal',"labels_hat_all", 'testing_labels_all');
         end
