...
%%%% to run this code you should have already binned and parameterized data for wake and sleep, BM and LI %%%%%
%%%% if this did not the case run until line 60 twice- once while WSA='wake', and once while WSA='sleep'
...
WSA='sleep';
block_length=5;
block_length_in_minutes=block_length;
...
%parameters=[{[8,15, 12,11, 29]}]%,{[8 9 10 11 28 29]}]%,{Result(1:3)'},{Result(1:7)'},[3,8,11,15,18]];%[1:30];
%parameters=Result.Ranking(1:5);%[1:30];
parameters=[{[8,15,14, 12,11,29]}]%,{[8 9 10 11 28 29]}]%,{Result(1:3)'},{Result(1:7)'},[3,8,11,15,18]];%[1:30];
...
% %%
SeparateNost=false;
% prepare_blocks_per_subject_from_git(directory_to_use,AllSubjData,WSA,normalize,SeparateNost,block_length_in_minutes,sliding_window_in_minutes,UseSelfReportedTimings)
...
% fprintf('prepared all raw data into blocks\n')
% 
 % create_LI_per_sbj_git(directory_to_use,AllSubjData,SeparateNost,WSA,normalize,block_length_in_minutes)
 % fprintf('LI parameters were calculated for all data blocks\n')

%%
WSA='wake';
[mean_wake_values,shorts_wake]=calculate_mean_BM_values_git(AllSubjData,directory_to_use,block_length,WSA,timings,start,normalize,overlap,cleaning,exclude_shorts);
mean_LI_wake_values=calculate_mean_LI_values_git(AllSubjData,directory_to_use,block_length,WSA,timings,start,normalize,overlap,cleaning,exclude_shorts);

WSA='sleep';
[mean_sleep_values,shorts_sleep]=calculate_mean_BM_values_git(AllSubjData,directory_to_use,block_length,WSA,timings,start,normalize,overlap,cleaning,exclude_shorts);
mean_LI_sleep_values=calculate_mean_LI_values_git(AllSubjData,directory_to_use,block_length,WSA,timings,start,normalize,overlap,cleaning,exclude_shorts);
...
for iii=1:size(mean_sleep_values,2)
    ...
    if classification
        ...
        mean_w_values_array(:,17)=[];
        mean_s_values_array(:,17)=[];

        for p=1:size(parameters,2)

            for ii=1:size(AllSubjData,1)

                testing=ii;
                training=setdiff(1:size(AllSubjData,1),testing);
    ...
    if ismember(100,timings)
        if size(parameters{1},2)>=3
            ...
            scatter3(full_wake(:,param(1)),full_wake(:,param(2)),full_wake(:,param(3)),50,'filled','MarkerFaceColor','#C30010');
                        [units] =add_unit24(param(1));
            xlabel([var_names{param(1)} ' ' units]);
                                   [units] =add_unit24(param(2));
            ylabel([var_names{param(2)}  ' ' units]);
                                    [units] =add_unit24(param(3));
