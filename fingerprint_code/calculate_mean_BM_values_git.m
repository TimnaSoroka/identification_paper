function[mean_values_all,excluded] =calculate_mean_BM_values_git(AllSubjData,directory_to_use,block_length,WSA,timings,start,normalize,overlap,cleaning,exclude_shorts)
...
for iii=1:length(timings)
    ...
    for sbj=1:size(AllSubjData,1)
        ...
        if normalize
            file=dir([ directory_to_use '/code/breathMetric_parameters/BM_param_' num2str(block_length) 'min_blocks_per_subj/' SubjectName '/' SubjectName '_zelano_' WSA '_*normalized.mat']);
        else
            file=dir([directory_to_use '/Data/sum_' num2str(block_length) 'min_blocks_per_subj/' SubjectName '/' SubjectName '*BM_' WSA '.mat']);
        end
        ...
        if size(x,1)-start+1<start+time_in_blocks-1
            if exclude_shorts
        ...
        elseif timings~=100
            x=x(start:time_in_blocks+start-1,:);
        end
