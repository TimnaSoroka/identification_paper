function [raw_in_block]= data_into_blocks(block_length_in_minutes,sliding_window,Data_wake,SeparateNost,normalize,StartPoint,how_long)
...
if SeparateNost
    if size(Data_wake,2)==2
        raw_data=Data_wake(:,1);
    else
    raw_data=Data_wake(StartPoint:end,1);
    end
else
    if size(Data_wake,2)==2 
