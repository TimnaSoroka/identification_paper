...
timings=true;
plot3D=false;
wake_sleep_capped=false; %can run only after creating 5 minutes blocks for both wake and sleep
...
for i=1:nn
    % run_BM_per_sbj_minimum(AllSubjData,directory_to_use,WSA,normalize,...
    % overlap,1,block_length_in_minutes,train_by_hours,prc_train,timings,start,rand)
run_BM_per_sbj_minimum2025_3D_git(AllSubjData,directory_to_use,WSA,normalize,...
    overlap,1,block_length_in_minutes,train_by_hours,prc_train,timings,start,rand)
end
end
end

if wake_sleep_capped
prc=0.65;%,0.7,0.8,0.85];
for p=1:size(prc,2)
    prc_train=prc(p);
%timings=[60;120;180;360;540;100];
...
for i=1:nn
    % run_BM_per_sbj_minimum(AllSubjData,directory_to_use,WSA,normalize,...
    % overlap,1,block_length_in_minutes,train_by_hours,prc_train,timings,start,rand)
run_BM_per_sbj_minimum_sleepwake_2025_git(AllSubjData,directory_to_use,WSA,normalize,...
    overlap,1,block_length_in_minutes,train_by_hours,prc_train,timings,start,rand)
end
end
end