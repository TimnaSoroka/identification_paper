function acc_id=run_BM_per_sbj_minimum2025_3D(AllSubjData,directory,WSA,normalize,overlap,cleaning,block_length,train_by_hours,prc_train,timings,start,rand)
...
if ~rand
    rng(500) %1=86.6 %100=87.62 %500=90.72 %1500=88.66
end
...
for iii=1:length(timings)
    ...
    for i=1:size(AllSubjData,2)
        ...
        if normalize
            ...
            file=dir([directory '/code/breathMetric_parameters/BM_param_' num2str(block_length) 'min_blocks_per_subj/' SubjectName '/' SubjectName '*zelano_' WSA '*p.mat']);
        end
...
zelano_test(:,17)=[];
    ...
    %     else
            save([directory '/newFigs/MRMR/mrmr_acc_id_zelano_'  WSA '_' num2str(num_param) 'parameters_' num2str(prc_train) '_' num2str(timings(iii)) 'min_no_overlap_' num2str(size(AllSubjData,2)) '.mat'],'C','validationAccuracyReal',"labels_hat", 'testing_labels','num_param');
    %     end
    % end
end
    end
