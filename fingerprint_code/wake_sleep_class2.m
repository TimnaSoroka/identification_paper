    ...
    if classification
       ...
       % for p=1:size(parameters,2)
            ...
            for ii=1:size(AllSubjData,2)
                ...
                [~,p_to_sort]=ttest(mean_s_values_array(training,1:end-1),mean_w_values_array(training,1:end-1));
                [~,idx_p]=sort(p_to_sort);
parameters=[Result(1:5)];
[mdl, validationAccuracyReal] = linearDisc(training_mat(:,parameters),training_labels);
                labels_hat = mdl.predictFcn(testing_mat(:,parameters));
                C = confusionmat(testing_labels,labels_hat);
                acc(ii) = sum(diag(C)) / sum(C(:));
                clearvars mdl
            end
acc_vec{iii}=acc;
            mean_acc(iii)=mean(mean(acc,2))*100;
            mean_std(iii)=mean(std(acc,0,2))*100;
            SEs(iii)=mean_std(iii)./sqrt(size(mean_s_values_array,1)-1);
        ...
        if save_results
            save([ directory_to_use '/newFigs/wake_sleep_timings'],"SEs","mean_acc","mean_std","timings","parameters",'acc_vec');
        end
    end
end
...
full_time_mean_wake_values(:,17)=[];
full_time_mean_sleep_values(:,17)=[];
full_time_mean_wake_values(:,17+24)=[];
full_time_mean_sleep_values(:,17+24)=[];
...
if plot_timings_fig && classification
    ...
    % errorbar(mean_acc, SEs,  'color' , [0.21 0.27 0.31],'linewidth' , 1.2); %'lineStyle' , 'none',
        plot(mean_acc,'k','MarkerSize',10,'MarkerFaceColor','black', 'MarkerEdgeColor','auto')
    xl=[];
