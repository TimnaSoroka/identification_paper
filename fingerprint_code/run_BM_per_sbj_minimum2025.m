function C=run_BM_per_sbj_minimum2025(AllSubjData,directory,WSA,normalize,overlap,cleaning,block_length,train_by_hours,prc_train,timings,start,rand)
...
for iii=1:length(timings)
    ...
    params=[1:16,18:25];
        ...
        [mdl, validationAccuracyReal] = mediumNeuralNetworkK(zelano_mat(:,params),zelano_labels);
        labels_hat = mdl.predictFcn(zelano_test(:,params));
        C = confusionmat(zelano_testing_labels,labels_hat);
        labels_hat_all=labels_hat;
        testing_labels_all=zelano_testing_labels;
