function acc_id=run_BM_per_sbj_minimum_option2(directory,WSA,normalize,overlap,cleaning,block_length,train_by_hours,prc_train,timings,start,rand)
%load('/Users/worg/Documents/lior_data/Timna/QA_updated4.mat');
%AllSubjData=AllSubjData(1:97);
load('/Users/worg/Documents/fingerprint/DATA_97.mat');
AllSubjData=DATA_97;
%%
...
for iii=1:length(timings)
    ...
    for i=1:size(AllSubjData,2)
        ...
        SubjectName=subjData.SerialCode;
        %SubjectName=subjData.Code;
...
%x=subjData.Data_wake;
currentState=['BM_' WSA];
mat=subjData.(currentState);
        ...
        for ii=1:length(o)
           ...
           x=struct2table(mat);
            ...
            elseif time_to_check==100
                D=Data_per_subject;
               % L=labels;
L=repmat({SubjectName},size(D,1),1);
    ...
    if rand
        if overlap
            save([directory '/result_BM_same_day/acc_id_zelano_' WSA '_' num2str(block_length) 'min_blocks_' num2str(prc_train) '_' num2str(timings(iii)) 'min_rand_' num2str(size(AllSubjData,2)) '_' num2str(datetime) '.mat'],"acc",'acc_id','validationAccuracyReal',"labels_hat", 'zelano_testing_labels');
        else
            save([directory '/result_BM_same_day/acc_id_zelano_' WSA '_' num2str(block_length) 'min_blocks_'  num2str(prc_train) '_' num2str(timings(iii)) 'min_no_overlap_rand_' num2str(size(AllSubjData,2)) '_' datestr(datetime) '.mat'],"acc",'acc_id','validationAccuracyReal',"labels_hat", 'zelano_testing_labels');
        end
    else
        if overlap
            save([directory '/result_BM_same_day/acc_id_zelano_' WSA '_' num2str(block_length) 'min_blocks_' num2str(prc_train) '_' num2str(timings(iii)) 'min_' num2str(size(AllSubjData,2)) '.mat'],"acc",'acc_id','validationAccuracyReal',"labels_hat", 'zelano_testing_labels');
        else
            save([directory '/result_BM_same_day/acc_id_zelano_'  WSA '_' num2str(block_length) 'min_blocks_' num2str(prc_train) '_' num2str(timings(iii)) 'min_no_overlap_' num2str(size(AllSubjData,2)) '.mat'],"acc",'acc_id','validationAccuracyReal',"labels_hat", 'zelano_testing_labels');
        end
    end
end
end